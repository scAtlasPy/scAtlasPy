...
#========== todo 归一化 normalize
#========== todo 归一化 normalize 法 1 ： 在 X_CSR_data 表上直接处理 ， 含替换原数据 ==========
def normalize_total(atlas: Atlas,
    ...
    #             CASE
    #                 WHEN s.total > 0
    #                 THEN x.data * {float(target_sum)} / s.total
    #                 ELSE 0
    #             END
    #         FROM _cell_sum AS s
    #         WHERE x.cell_index = s.cell_index
    #     """)
    if inplace:
        print("原地修改 X_CSR_data.data ...")

        # 步骤1: 添加临时字段 temp_normalize 并计算归一化值
        conn.execute("""
               ALTER TABLE X_CSR_data
               ADD COLUMN IF NOT EXISTS temp_normalize REAL
           """)
        ...
        # 步骤3: 将 temp_normalize 字段重命名为 data
        conn.execute("""
               ALTER TABLE X_CSR_data
               RENAME COLUMN temp_normalize TO data
           """)
...
#========== todo 归一化 normalize 法 2 ： 在 X_CSR_data 表上直接处理 ， 不替换原数据 ==========
...
#========== todo log1p  法 1 ： 在 X_CSR_data 表上直接处理 ==========
...
#========== todo normalize_and_log1p： 法 3 + log1p 法 2 ==========
...
# todo === 进行 z-score转换 - 在 X_CSR 表上进行操作
def scale(
        atlas: Atlas,
...
# ):
#     """
#     Gene-wise z-score scale（DuckDB 优化版，1e8 cell 安全）
#
#     核心思想：
#     - gene 分块算 mean / std
#     - 每个 gene chunk 生成临时 scale 结果
#     - 最后一次性 merge 回 X_CSR_data
#     """
#
#     print("==== scale_gene_chunked_vA (OLAP optimized) ====")
#     start = datetime.now()
#     conn = atlas.connection
#
#     # -------------------------------------------------
...
#
#         if hvg_exists == 0:
#             raise ValueError(f"var 表中不存在字段: {hvg_key}")
#
...
#         avg = elapsed / max(i, 1)
#         remain = avg * (n_chunks - i)
#
#         print(
#             f"\n[Chunk {i + 1}/{n_chunks}] "
#             f"genes={len(chunk_genes)} | "
#             f"elapsed={timedelta(seconds=int(elapsed))} | "
#             f"ETA={timedelta(seconds=int(remain))}"
#         )
#
#         # ---------------------------------------------
...
#
#     # -------------------------------------------------
#     # 6. 一次性 merge 回主表（只 UPDATE 一次）
#     # -------------------------------------------------
#     print("Merging scaled data back to X_CSR_data ...")
#
#     conn.execute(f"""
#         UPDATE X_CSR_data AS x
#         SET {add_field} = t.val
#         FROM _X_scale_tmp AS t
#         WHERE x.id = t.id
#           AND x.indices = t.indices
#     """)
#
#     print("\nscale_gene_chunked_vA 完成")
