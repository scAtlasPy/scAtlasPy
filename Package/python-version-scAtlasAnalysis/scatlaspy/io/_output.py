...
def export_duckdb_to_h5ad(
    ...
    *,  # * 后面的参数只能用关键字参数
    ...
    obs = conn.execute("""
        SELECT * FROM obs ORDER BY atlas_cell_id
    """).df()

    var = conn.execute("""
        SELECT * FROM var ORDER BY atlas_gene_id
    """).df()

    # ★ NOTE: AnnData 要求 index 是 atlas_cell_id / atlas_gene_id
    obs = obs.set_index("atlas_cell_name")
    var = var.set_index("atlas_gene_name")
