...
class CSRBatchFetcherMT:
    ...
    def _consumer(self):
        """
           ...
           - 每 5 个 batch 输出一次实时调试信息
        """

        conn = duckdb.connect(self.db_path, read_only=True)  # consumer 独立 conn

        # 获取 indptr
        fetch_record_indptr = (
            conn.execute("SELECT indptr FROM X_CSR_indptr")
        ...
        next_seq_id = 0
        t_start = time.time()
        batch_counter_since_last_report = 0  # 计数器，每 5 batch 输出一次
        ...
        while True:
            ...
            # 只按 seq_id 顺序消费
            while next_seq_id in buffer:
                indices, vals = buffer.pop(next_seq_id)
                self.pool_indices = np.concatenate([self.pool_indices, indices])
                self.pool_data = np.concatenate([self.pool_data, vals])
                ...
                while self.batch_idx < len(self.batch_nnz):
                    ...
                    data_vals = self.pool_data[:need]
                    n_cells = self.batch_size
                    n_genes = int(cols.max()) + 1
                    ...
                    X = sp.csr_matrix((data_vals, cols, indptr_now), shape=(n_cells, n_genes), dtype=np.float32)

                    # -----------------------------
                    # 每 5 个 batch 输出一次调试信息
                    # -----------------------------
                    batch_counter_since_last_report += 1
                    if batch_counter_since_last_report >= 5:
                        now = time.time()
                        total_elapsed = now - t_start
                        bps = batch_counter_since_last_report / total_elapsed  # 平均 batch/s
                        print(f"[Consumer Debug] batch {self.batch_idx}, nnz={need}, "
                              f"total_elapsed={total_elapsed:.2f}s, avg_batch/s={bps:.2f}")
                        batch_counter_since_last_report = 0
                        t_start = now  # 重置计时器
