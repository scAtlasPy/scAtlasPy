...
class CSRBatchFetcherMP_SharedMemNoQueue:
    ...
    def __init__(self, db_path, batch_size=2048, producer_num=4, slot_num=10):
        ...
        # 🔹 私有 内存池 consumer pool ,预分配一个足够大的 buffer（4 倍最大 batch，防止频繁搬移）
        self.pool_capacity = max_nnz * 4
        self.pool_indices = np.empty(self.pool_capacity, dtype=np.int32)
        self.pool_data = np.empty(self.pool_capacity, dtype=np.float32)
        self.pool_start = 0  # 已消费起点
        self.pool_end = 0  # 已写入终点
    ...
    def _consumer(self):
        ...
        while self.batch_idx < self.batch_num :
            ...
            # todo data 数量够， 取 数据
            if (self.pool_end - self.pool_start) >= need: # 私有内存池中的data 数量足够

                # 从内存pool中获取 need 个 数据
                vals = self.pool_data[self.pool_start:self.pool_start + need] # csr三元组 1 — data
                cols = self.pool_indices[self.pool_start:self.pool_start + need] # csr三元组 2 — gene_id
                ...
                # 移动 pool（丢弃已消费的 nnz）
                self.pool_start += need

                # 如果前面空洞太大，做一次整体搬移（很少发生）
                if self.pool_start > self.pool_capacity // 2:
                    remaining = self.pool_end - self.pool_start
                    self.pool_indices[:remaining] = self.pool_indices[self.pool_start:self.pool_end]
                    self.pool_data[:remaining] = self.pool_data[self.pool_start:self.pool_end]

                    self.pool_start = 0
                    self.pool_end = remaining
            ...
            # todo data 数量不够， 存 数据
                ...
                # 如果空间不够，扩容一次（极少发生）
                if self.pool_end + length > self.pool_capacity:
                    new_capacity = self.pool_capacity * 2
                    new_indices = np.empty(new_capacity, dtype=np.int32)
                    new_data = np.empty(new_capacity, dtype=np.float32)

                    current_size = self.pool_end - self.pool_start
                    new_indices[:current_size] = self.pool_indices[self.pool_start:self.pool_end]
                    new_data[:current_size] = self.pool_data[self.pool_start:self.pool_end]

                    self.pool_indices = new_indices
                    self.pool_data = new_data
                    self.pool_capacity = new_capacity
                    self.pool_start = 0
                    self.pool_end = current_size

                # 直接写入尾部
                self.pool_indices[self.pool_end:self.pool_end + length] = indices
                self.pool_data[self.pool_end:self.pool_end + length] = data
                self.pool_end += length
