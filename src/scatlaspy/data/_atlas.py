...
class Atlas:
    ...
    def query(self, query: str) -> pd.DataFrame:
        ...
        Count cells by cluster::
            ...
            atlas.query(
                "SELECT scatlas_cluster, COUNT(*) AS n_cells "
                "FROM obs GROUP BY scatlas_cluster ORDER BY scatlas_cluster"
            )
        """
        ...
        *,
        ...
        if table_exists == 0:
            ...
            print(
                ...
                "Please rerun PCA, clustering, and other operations that depend on this table!")
    ...
    def build_read_index(
        ...
        condition, and highly variable gene marker. PCA, clustering, and some plotting
    ...
    def get_minibatch_dense(
        ...
        streaming randomized PCA, clustering, streaming training, and
        ...
        ``obs`` table, such as ``scatlas_cluster`` labels, is retrieved based on these
        ``filter_cell_ids``.
        ...
        get_obs_col
            ...
            for example ``"scatlas_cluster"``.
            Defaults to ``None``, meaning no ``obs`` column is queried and each
            iteration only yields ``X_batch``.

            When a column name is provided, such as ``get_obs_col="scatlas_cluster"``, this
            method automatically asks the underlying minibatch reader to return
            ``filter_cell_ids``, then queries ``obs.scatlas_cluster`` according to these IDs,
            and returns a dictionary:

            ``{"X": X_batch, "filter_cell_ids": filter_cell_ids, "scatlas_cluster": values}``
        ...
        Return ``obs.scatlas_cluster`` labels together::
    ...
    def workflow_state(self) -> pd.DataFrame:
        ...
        rows = [
            ...
            {
                "artifact": "scatlas_cluster",
                "present": self.has_column("obs", "scatlas_cluster"),
                "evidence": "obs.scatlas_cluster",
                "meaning": "Default scAtlasPy cluster labels have been stored.",
            },
            {
                "artifact": "umap",
