...
def _prepare_cold_start_prefixes(
    block_starts: np.ndarray,
    n_cells: int,
    last_block_start: int,
    carry_target_cells: int,
    cold_block_limit: int = 100,
) -> tuple[dict[int, int], int, int]:
    """Choose cold-start block prefixes for the initial rolling carry."""

    # Cold start fills the initial carry from many random blocks. This avoids
    # the first written window being dominated by only a few neighboring cells.
    cold_block_count = min(cold_block_limit, len(block_starts))

    # Split carry_target_cells across cold_block_count blocks. The first
    # cold_extra_cells blocks receive one extra cell so the total is exact.
    cold_base_cells = carry_target_cells // cold_block_count
    cold_extra_cells = carry_target_cells % cold_block_count

    # The largest cold prefix length. Only the original last block can be too
    # short because block_starts is generated by np.arange.
    cold_max_cells = cold_base_cells + int(cold_extra_cells > 0)
    cold_candidate_starts = block_starts
    if n_cells - last_block_start < cold_max_cells:
        cold_candidate_starts = block_starts[block_starts != last_block_start]

    # Each cold start reads a prefix of a block, so normal reading only needs
    # to resume from prefix_end for those same block starts.
    cold_prefix_end_by_start = {}
    for cold_i, block_start in enumerate(cold_candidate_starts[:cold_block_count]):
        block_start = int(block_start)
        prefix_cells = cold_base_cells + int(cold_i < cold_extra_cells)
        cold_prefix_end_by_start[block_start] = block_start + prefix_cells

    return cold_prefix_end_by_start, cold_base_cells, cold_extra_cells
...
def _load_h5ad_random(
    ...
    _, blocks_per_pool = _estimate_window_cells_and_blocks_per_pool(
        memory_limit=atlas.db_memory_limit,
        cells_per_block=cells_per_block,
    ...
    # The window is capped by n_cells so small datasets still use meaningful
    # rolling targets.
    window_cells = min(cells_per_block * blocks_per_pool, n_cells)

    # carry_target_cells is the number of shuffled cells retained for the next
    # round; write_target_cells is the number written in the current round.
    carry_target_cells = max(1, window_cells // 2)
    write_target_cells = max(1, window_cells - carry_target_cells)
    total_windows = math.ceil(n_cells / write_target_cells)

    (
        cold_prefix_end_by_start,
        cold_base_cells,
        cold_extra_cells,
    ) = _prepare_cold_start_prefixes(
    ...
    try:
        ...
        cold_blocks = []
        for block_start, block_end in cold_prefix_end_by_start.items():
            if block_start >= block_end:
                continue
            adata = block_reader.read_block(block_start, block_end)
            adata = _convert_obs_categories_to_object_inplace(adata)
            adata = _convert_x_to_count_inplace(
                adata,
                source_x_scale=source_x_scale,
            )
            cold_blocks.append(adata)

        if cold_blocks:
            carry_adata = _concat_and_shuffle_adatas(cold_blocks)
        ...
        for block_i, block_start in enumerate(
            ...
            block_start = int(block_start)
            block_end = min(block_start + cells_per_block, n_cells)
            read_start = cold_prefix_end_by_start.get(block_start, block_start)

            if read_start >= block_end:
                continue

            adata = block_reader.read_block(read_start, block_end)
            adata = _convert_obs_categories_to_object_inplace(adata)
            adata = _convert_x_to_count_inplace(
                adata,
                source_x_scale=source_x_scale,
            ...
            if carry_cells_now + new_cells >= window_cells:
                ...
                mixed_cells = result["mixed_cells"]
...
def _concat_and_shuffle_adatas(adatas: list[AnnData]) -> AnnData:
    """Concatenate prepared AnnData chunks and shuffle cells uniformly."""

    t_category0 = time.time()
    for i, adata in enumerate(adatas):
        adatas[i] = _convert_obs_categories_to_object_inplace(adata)
    t_category = time.time() - t_category0
...
def _write_prepared_adata_to_duckdb(
    adata_window: AnnData,
    conn: DuckDBPyConnection,
    global_cell_id: int,
    global_indptr_id: int,
    global_indptr_offset: int,
...
def _write_rolling_shuffle_window_to_duckdb(
    carry_adata: AnnData | None,
    ...
    if carry_adata is not None and carry_adata.n_obs > 0:
        parts.append(carry_adata)
    parts.extend(new_adatas)

    adata_window = _concat_and_shuffle_adatas(parts)
    mixed_cells = adata_window.n_obs
    ...
    (
        ...
        written_cells,
        written_nnz,
    ) = _write_prepared_adata_to_duckdb(
        adata_window=write_adata,
        conn=conn,
        global_cell_id=global_cell_id,
        global_indptr_id=global_indptr_id,
        global_indptr_offset=global_indptr_offset,
