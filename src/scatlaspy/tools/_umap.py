...
def _import_parametric_umap() -> Any:
    ...
    try:
        ...
        raise ImportError(
            "sap.tl.umap requires the optional ParametricUMAP dependencies, "
            "including TensorFlow. Install them in your analysis environment "
            "with `pip install 'scatlaspy[parametric]'`, then call this "
            "function again."
        ) from exc

    _patch_parametric_umap_loss()
    return ParametricUMAP


def _patch_parametric_umap_loss(eps: float = 1e-6) -> None:
    """Patch ParametricUMAP's loss to avoid NaN gradients at zero distance."""

    try:
        import keras
        import tensorflow as tf
        from keras import ops
        from umap.parametric_umap import (
            UMAPModel,
            compute_cross_entropy,
            convert_distance_to_log_probability,
        )
    except ImportError as exc:
        raise ImportError(
            "ParametricUMAP requires TensorFlow and Keras. Install the "
            "parametric extra with `pip install 'scatlaspy[parametric]'`."
        ) from exc

    if getattr(UMAPModel, "_scatlaspy_safe_loss_patched", False):
        return

    def safe_umap_loss(self: Any, y_pred: dict[str, Any], repulsion_strength: float = 1.0) -> Any:
        ...
        def safe_distance(diff: Any) -> Any:
            return ops.sqrt(ops.sum(ops.square(diff), axis=1) + eps)

        distance_embedding = ops.concatenate(
            [
                safe_distance(embedding_to - embedding_from),
                safe_distance(embedding_neg_to - embedding_neg_from),
            ],
            axis=0,
        )
    ...
    UMAPModel._umap_loss = safe_umap_loss
    UMAPModel._scatlaspy_safe_loss_patched = True
...
def _tensorflow_device_context(device: str | int | None) -> tuple[Any, str]:
    """Return a TensorFlow device context factory for ParametricUMAP."""

    if device is None:
        return nullcontext, "auto"

    try:
        import tensorflow as tf
    except ImportError as exc:
        raise ImportError(
            "The device parameter for sap.tl.umap requires TensorFlow. Install "
            "the parametric extra with `pip install 'scatlaspy[parametric]'`."
        ) from exc

    if isinstance(device, int):
        device_name = f"/GPU:{int(device)}"
    else:
        device_name = str(device).strip()
        if not device_name:
            raise ValueError("device must be None, an integer GPU id, or a TensorFlow device string")
        if device_name.upper() == "CPU":
            device_name = "/CPU:0"
    ...
    if not (upper_name.startswith("/CPU:") or upper_name.startswith("/GPU:")):
        raise ValueError(
            "device must be None, an integer GPU id, or a TensorFlow device "
            "string such as 'GPU:0', '/GPU:1', or 'CPU:0'"
        )

    if upper_name.startswith("/GPU:"):
        try:
            gpu_id = int(device_name.split(":")[-1])
        except ValueError as exc:
            raise ValueError(f"Invalid GPU device string: {device}") from exc

        gpus = tf.config.list_physical_devices("GPU")
        if gpu_id < 0 or gpu_id >= len(gpus):
            raise ValueError(
                f"Requested device {device_name}, but TensorFlow sees {len(gpus)} GPU device(s)"
            )

    return lambda: tf.device(device_name), device_name
...
def umap(
    ...
    """Calculate atlas-scale UMAP coordinates with ParametricUMAP.

    ``sap.tl.umap`` uses ParametricUMAP as the default and only public UMAP
    backend. It trains a neural mapping from sampled PCA coordinates and then
    predicts UMAP coordinates for all cells in batches, which is the scalable
    path used by scAtlasPy for atlas-scale data.
    """
    ...
    return parametric_umap(
        ...
        save_eval_table=save_eval_table,
...
def parametric_umap(
    ...
    batches. It is intended for atlas-scale datasets where graph-optimization
    based transformation is too slow.
    ...
    device
        TensorFlow device used for ParametricUMAP. Use ``None`` for TensorFlow's
        default placement, an integer GPU id such as ``0``, or a device string
        such as ``"GPU:0"`` or ``"CPU:0"``.
    ...
    device_context, device_name = _tensorflow_device_context(device)
